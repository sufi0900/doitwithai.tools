...
async function getData(page = 1, pageSize = 5) {
  const skip = (page - 1) * pageSize;
  const query = `*[_type == "blog"] | order(publishedAt desc)[${skip}..${
    skip + pageSize - 1
  }]`;
...
export default function AllPosts() {
  ...
  console.log("Query:", router.query); // Log the query object
  const { query } = router;
  const initialPage = query && query.page ? parseInt(query.page) : 1; // Add null check for query
  ...
  useEffect(() => {
    async function fetchData() {
      const newData = await getData(currentPage, pageSize);
      setData(newData);
    }
