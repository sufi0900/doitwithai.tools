...
const BlogHeader = ({ data, imgdesc, articleLoading = false }) => {
 const { isRefreshing } = usePageRefresh();
  ...
  useEffect(() => {
    ...
    if (isRefreshing || articleLoading || imageChanged) {
      setTitleLoading(true);
      setTitleVisible(false);
      
      // Only reset image specific loading if the image actually changed,
      // or if it's a full article re-load (not just a minor data refresh).
      // We check !isRefreshing here to avoid resetting if the data itself is stable
      // but the page context just says it's "refreshing".
      if (imageChanged || articleLoading) { // Changed this condition
        setIsLoading(true);
        ...
        // If the image itself didn't change and it's just a page-level refresh,
        // try to keep the image visible if it was already loaded.
        // We might still briefly show loading for other elements, but not the image.
  ...
  }, [isRefreshing, articleLoading, data?.mainImage?.asset?._ref]); // Add image asset ref to dependencies

  // Title loading animation (unchanged, still applies on refresh)
  useEffect(() => {
    if (data?.title && !isRefreshing && !articleLoading) {
  ...
  }, [data?.title, isRefreshing, articleLoading]);

  // Hide static placeholder after delay (unchanged)
  useEffect(() => {
    if (isRefreshing || articleLoading) return;
    const timer = setTimeout(() => {
      setShowStaticPlaceholder(false);
    }, 500);
    return () => clearTimeout(timer);
  }, [isRefreshing, articleLoading]);
  ...
  }, [inView, isLoading, imageVisible]); // Added imageVisible to dependencies

  // Show loading overlay when article is refreshing
  // This now primarily controls the title/text content overlay,
  // not necessarily the image if the image didn't change.
  const shouldShowLoadingOverlay = (isRefreshing || articleLoading) && data;
  return (
