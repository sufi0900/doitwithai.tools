...
const ResourceModal = ({ resource, isOpen, onClose }) => {
  ...
  const handleResourceAccess = () => {
    if (resource.resourceFormat === 'text' && resource.promptContent) return; // For text, we handle copy separately
  ...
  const renderStarRating = (rating) => {
    ...
    for (let i = 1; i <= 5; i++) {
      stars.push(
        <svg
          key={i}
          className={`w-5 h-5 transition-all duration-300 ${i <= rating ? 'text-yellow-400 scale-110' : 'text-gray-300 dark:text-gray-600 hover:text-yellow-200'}`}
          fill="currentColor"
          viewBox="0 0 20 20"
        >
          <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.073 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.82 2.034a1 1 0 00-.364 1.118l1.073 3.292c.3.921-.755 1.688-1.541 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.82 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.988 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
        </svg>
      );
    }
    return (
      <div className="flex items-center gap-1 bg-gradient-to-r from-yellow-50 to-orange-50 dark:from-yellow-900/20 dark:to-orange-900/20 px-3 py-1.5 rounded-full border border-yellow-200 dark:border-yellow-800">
        {stars}
        <span className="text-sm font-medium text-yellow-700 dark:text-yellow-300 ml-2">{rating}/5</span>
      </div>
    );
  };
  ...
  return (
    <div
      ...
      itemScope
      itemType={`https://schema.org/${resource.resourceFormat === 'image' ? 'ImageObject' : resource.resourceFormat === 'video' ? 'VideoObject' : resource.resourceFormat === 'text' ? 'TextDigitalDocument' : resource.resourceFormat === 'aitool' ? 'SoftwareApplication' : 'DigitalDocument'}`}
    ...
    >
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl max-w-5xl w-full max-h-[95vh] overflow-hidden flex flex-col shadow-2xl border border-gray-200 dark:border-gray-800 animate-slideUp">
        {/*Enhanced Header - Adjusted padding and text sizes*/}
        <div className={`relative ${isAITool ? 'bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600' : 'bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900'} px-5 py-4 sm:px-6 sm:py-4`}>
          {isAITool && (
          ...
          <div className="relative flex justify-between items-start">
            <div className="flex-1 pr-4">
              <div className="flex items-center gap-2 mb-2"> {/* Adjusted gap and mb */}
                <div className={`inline-flex items-center px-2 py-1 rounded-full text-xs sm:text-sm font-medium ${isAITool ? 'bg-white/20 text-white backdrop-blur-sm border border-white/30' : 'bg-blue-500/20 text-blue-800 dark:bg-blue-500/30 dark:text-blue-300'}`}>
                  {isAITool && <span className="mr-1">🤖</span>} {/* Adjusted mr */}
                  {resource.resourceType}
                </div>
                {isAITool && resource.aiToolDetails?.toolCategory && (renderCategoryBadge(resource.aiToolDetails.toolCategory))}
              </div>
              <h3 className={`text-2xl sm:text-3xl font-bold mb-3 ${isAITool ? 'text-white' : 'text-gray-900 dark:text-white'}`} itemProp="headline">{resource.title}</h3> {/* Adjusted text size and mb */}
              <div className="flex items-center gap-3 flex-wrap"> {/* Adjusted gap */}
            ...
            <button
              ...
              className={`p-2 rounded-full transition-all duration-200 hover:rotate-90 ${isAITool ? 'text-white/80 hover:text-white hover:bg-white/20' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 hover:bg-gray-200 dark:hover:bg-gray-700'}`}
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {isAITool && resource.aiToolDetails ? (
              ...
              <div className="relative">
                <div className="relative aspect-[16/9] bg-gradient-to-br from-blue-100 via-indigo-50 to-purple-100 dark:from-blue-900/30 dark:via-indigo-900/30 dark:to-purple-900/30 rounded-xl overflow-hidden shadow-inner">
                  {resource.previewSettings?.previewImage ? (
                    <img src={urlForImage(resource.previewSettings.previewImage).url()} alt={resource.previewSettings.previewImage?.alt || altText} className="w-full h-full object-cover" itemProp="image" />
                  ) : resource.mainImage ? (
                    <img src={urlForImage(resource.mainImage).url()} alt={resource.mainImage?.alt || altText} className="w-full h-full object-cover" itemProp="image" />
                  ) : (
                      ...
                      <div className="absolute bottom-4 left-4 right-4">
                        <div className="bg-white/10 backdrop-blur-md rounded-lg p-3 border border-white/20">
                          <div className="flex items-center gap-2 text-white">
                            <svg className="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.11.1" /></svg>
                ...
                <nav className="flex space-x-8">
                  {[{ id: 'overview', label: 'Overview', icon: '📋' }, { id: 'features', label: 'Features', icon: '⭐' }, { id: 'pros-cons', label: 'Pros & Cons', icon: '⚖️' }].map((tab) => (
                    <button
                      ...
                      className={`py-2 px-1 border-b-2 font-medium text-sm transition-all duration-200 ${activeTab === tab.id ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-gray-400 dark:hover:text-gray-300'}`}
                    >
                  ...
                  <div className="space-y-6">
                    {resource.overview && (
                      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 p-6 rounded-xl border border-blue-200 dark:border-blue-800">
                        <h4 className="text-lg font-semibold mb-3 text-gray-900 dark:text-white flex items-center"><span className="mr-2">📝</span>Overview</h4>
                        <p className="text-gray-700 dark:text-gray-300 leading-relaxed text-lg" itemProp="description">{resource.overview}</p>
                      </div>
                    )}
                    {resource.aiToolDetails.functionality && resource.aiToolDetails.functionality.length > 0 && (
                      <div>
                        <h4 className="text-lg font-semibold mb-4 text-gray-900 dark:text-white flex items-center"><span className="mr-2">⚡</span>Key Functionality</h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                          {resource.aiToolDetails.functionality.map((func, index) => (
                            <div key={index} className="group bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-all duration-200 hover:scale-105">
                ...
                {activeTab === 'features' && resource.aiToolDetails.features && resource.aiToolDetails.features.length > 0 && (
                  <div className="grid gap-4 md:grid-cols-2">
                    {resource.aiToolDetails.features.map((feature, index) => (
                      <div key={index} className="group bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-lg transition-all duration-300 hover:scale-[1.02]">
                        <div className="flex items-start">
                          <div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-xl flex items-center justify-center mr-4 group-hover:scale-110 transition-transform duration-200">
                            <span className="text-white font-bold">#{index + 1}</span>
                          </div>
                          <div className="flex-1">
                            <h5 className="font-semibold text-gray-900 dark:text-white mb-2 text-lg">{feature.feature}</h5>
                            {feature.description && (
                        ...
                        <ul className="space-y-3">
                          {resource.aiToolDetails.prosAndCons.pros.map((pro, index) => (
                            <li key={index} className="flex items-start group">
                              ...
                              <span className="text-green-700 dark:text-green-300 leading-relaxed">{pro}</span>
                            </li>
                          ))}
                        </ul>
                      </div>
                      ...
                      <div className="bg-gradient-to-br from-red-50 to-pink-50 dark:from-red-900/20 dark:to-pink-900/20 p-6 rounded-xl border border-red-200 dark:border-red-800 shadow-sm">
                        <h5 className="font-bold text-red-800 dark:text-red-400 mb-4 flex items-center text-lg">
                          <div className="w-8 h-8 bg-red-500 rounded-full flex items-center justify-center mr-3">
                            <svg className="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" /></svg>
                          </div>
                          Limitations
                        </h5>
                        <ul className="space-y-3">
                          {resource.aiToolDetails.prosAndCons.cons.map((con, index) => (
                            <li key={index} className="flex items-start group">
                              ...
                              <span className="text-red-700 dark:text-red-300 leading-relaxed">{con}</span>
                            </li>
                          ))}
                        </ul>
                      </div>
            ...
            <div className="p-6">
                    ...
                    {resource.promptContent.map((promptItem, index) => (
                      <div key={index} className="bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 rounded-xl p-6 border border-gray-200 dark:border-gray-700 shadow-sm">
                        <div className="flex justify-between items-center mb-4">
                          <h4 className="text-lg font-semibold text-gray-900 dark:text-white flex items-center"><span className="mr-2">💬</span>{promptItem.promptTitle || `Prompt ${index + 1}`}</h4>
                          <button
                            onClick={() => handleCopy(promptItem.promptText, `prompt-${index}`)}
                            className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white py-2 px-4 rounded-lg transition-all duration-200 flex items-center shadow-md hover:shadow-lg transform hover:scale-105"
                          >
                            {copyStates[`prompt-${index}`] ? (
                              ...
                              <><svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-1M8 5a2 2 0 002 2h2a2 2 0 002-2M8 5a2 2 0 012-2h2a2 2 0 012 2m0 0h2a2 2 0 012 2v3m2 4H10m0 0l3-3m-3 3l3 3" /></svg>Copy Prompt</>
                            )}
                          </button>
                        </div>
                        <div className="bg-gradient-to-br from-gray-50 to-white dark:from-gray-800 dark:to-gray-900 p-5 rounded-xl border border-gray-200 dark:border-gray-700 shadow-inner">
                          <pre className="whitespace-pre-wrap font-mono text-sm text-gray-800 dark:text-gray-200 leading-relaxed overflow-x-auto" itemProp="text">{promptItem.promptText}</pre>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="relative">
                    <div className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 p-8 rounded-xl border border-blue-200 dark:border-blue-800 shadow-sm">
                      <div className="text-center">
                        ...
                        <h4 className="text-xl font-bold text-gray-900 dark:text-white mb-2">{resource.resourceFile.originalFilename || resource.title}</h4>
                        <p className="text-gray-600 dark:text-gray-400 mb-4">Document ready for download</p>
                        <div className="flex items-center justify-center gap-2 text-sm text-blue-600 dark:text-blue-400">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.96 6H16a5 5 0 011 9.9M9 19l3 3m0 0l3-3m-3 3V10" /></svg>
                ...
                {!['image', 'video', 'text', 'document', 'aitool'].includes(resource.resourceFormat) && (
                  <div className="bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 p-8 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                    <div className="text-center">
                      ...
                      <h4 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">Resource Available</h4>
                      <p className="text-gray-600 dark:text-gray-400">{resource.resourceType} • Ready for access</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              {resource.overview && (
                <div className="mb-8">
                  <div className="bg-gradient-to-r from-blue-50 via-indigo-50 to-purple-50 dark:from-blue-900/20 dark:via-indigo-900/20 dark:to-purple-900/20 p-6 rounded-xl border border-blue-200 dark:border-blue-800 shadow-sm">
                  ...
                  <div className="bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                    ...
                    <div className="prose dark:prose-invert max-w-none text-gray-700 dark:text-gray-300 prose-lg">
                      {typeof resource.content === 'string' ? (
                        ...
                        // If content is rich text, render it using a dedicated component/utility
                        // For simplicity, if it's not a string, we assume it's complex and just show a placeholder
              ...
              {resource.tags && resource.tags.length > 0 && (
                <div className="mb-8">
                  <h3 className="text-lg font-semibold mb-4 text-gray-900 dark:text-white flex items-center">
                    <svg className="w-6 h-6 mr-2 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" /></svg>
                    Tags
                  </h3>
                  <div className="flex flex-wrap gap-2">
                    {resource.tags.map((tag, index) => (
                      <span key={index} className="inline-flex items-center px-3 py-1.5 rounded-full text-sm font-medium bg-gradient-to-r from-blue-100 to-indigo-100 dark:from-blue-900/30 dark:to-indigo-900/30 text-blue-800 dark:text-blue-300 border border-blue-200 dark:border-blue-800 hover:scale-105 transition-transform duration-200">
          ...
          {/*Enhanced Related Article Section - Re-added*/}
          {resource.relatedArticle && resource.relatedArticle?.title && (
            <div className="mx-6 mb-6">
              <div className="bg-gradient-to-r from-teal-50 via-cyan-50 to-blue-50 dark:from-teal-900/20 dark:via-cyan-900/20 dark:to-blue-900/20 p-6 rounded-xl border border-teal-200 dark:border-teal-800 shadow-sm hover:shadow-md transition-all duration-300 group">
                <h3 className="text-lg font-semibold mb-3 text-gray-900 dark:text-white flex items-center">
                  <div className="w-8 h-8 bg-gradient-to-br from-teal-500 to-cyan-600 rounded-lg flex items-center justify-center mr-3 group-hover:scale-110 transition-transform duration-200">
                    <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.11.1" /></svg>
                  </div>
                  Related Article
                </h3>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      {resource.relatedArticle.excerpt && (
                        <p className="text-teal-600 dark:text-teal-400 text-sm leading-relaxed">{resource.relatedArticle.excerpt}</p>
                      )}
                    </div>
                    <div className="ml-4 text-teal-600 dark:text-teal-400 group-hover:text-teal-700 dark:group-hover:text-teal-300 transition-colors">
                      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2V14M14 4h6m0 0v6m0-6L10 14" /></svg>
                    </div>
                  </div>
                </a>
              </div>
          ...
          <div className="p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-4 text-sm">
              <div className="flex items-center text-gray-500 dark:text-gray-400">
                <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3a2 2 0 012-2h4a2 2 0 012 2v4m-6 0h6m6 0v6a2 2 0 01-2 2H8a2 2 0 01-2-2v-6m6 0V3" /></svg>
            ...
            <div className="flex gap-3">
              {resource.resourceFormat === 'text' && Array.isArray(resource.promptContent) && resource.promptContent.length > 0 ? (
                <button
                  ...
                  className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white py-3 px-6 rounded-xl transition-all duration-300 font-semibold flex items-center gap-2 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5"
                >
                  {copyStates['copy-all-prompts'] ? (
                    <><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>All Copied!</>
                  ) : (
                    <><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-1M8 5a2 2 0 002 2h2a2 2 0 002-2M8 5a2 2 0 012-2h2a2 2 0 012 2m0 0h2a2 2 0 012 2v3m2 4H10m0 0l3-3m-3 3l3 3" /></svg>Copy All Prompts</>
                  )}
                </button>
              ) : resource.resourceFormat === 'aitool' ? (
                <button
                  onClick={handleResourceAccess}
                  className="bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 hover:from-blue-700 hover:via-indigo-700 hover:to-purple-700 text-white py-3 px-8 rounded-xl transition-all duration-300 font-semibold flex items-center gap-3 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 group"
                ...
                >
                  ...
                  Launch AI Tool
                  <svg className="w-5 h-5 group-hover:translate-x-1 transition-transform duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2V14M14 4h6m0 0v6m0-6L10 14" /></svg>
                </button>
              ) : (
                <button
                  onClick={handleResourceAccess}
                ...
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={resource.resourceLinkType === 'external' ? "M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2V14M14 4h6m0 0v6m0-6L10 14" : "M7 16a4 4 0 01-.88-7.903A5 5 0 1115.96 6H16a5 5 0 011 9.9M9 19l3 3m0 0l3-3m-3 3V10"} />
                  </svg>
