...
export default function RootLayout({ children }) {
 ...
 // Remove the complex precaching logic and replace with this minimal version:
useEffect(() => {
  const handler = (e) => {
    ...
    // Only precache homepage and offline page - nothing else
    if (href === "/" || href === "/offline.html") {
      setTimeout(() => {
        navigator.serviceWorker?.controller?.postMessage({
          type: "PRECACHE_ESSENTIAL_ONLY",
          path: href,
        ...
        <div className="flex items-center justify-between space-x-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-gray-900 dark:text-gray-100">
              ...
              <p className="text-sm text-gray-700 dark:text-gray-300 mt-1">
                Cache: {typeof window !== 'undefined' && window.swCacheManager?.getStorageInfo()?.usage || 0}MB • 
                Mode: {typeof window !== 'undefined' && window.swCacheManager?.getDeviceType() || 'unknown'}
              </p>
            </div>
          </div>
          ...
          <div className="flex-shrink-0">
            <button
              onClick={() => {
                handleOfflineRetry();
                // Also trigger cleanup if needed
                if (typeof window !== 'undefined' && window.swCacheManager) {
                  window.swCacheManager.cleanup();
                }
              }}
